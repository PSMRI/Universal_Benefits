...
const { pool, exec_query_eg_payments_excel, create_eg_payments_excel, reset_eg_payments_excel, search_disbursal } = require("../api");
const { generatePaymentCSVFromDisbursals } = require("../utils/disbursalExportCsv");
...
router.post(
  ...
  asyncMiddleware(async function (req, res, next) {
    ...
    var tenantId = criteria?.tenantId;
    ...
    try {
      let disbursalRequest = {};
      disbursalRequest['RequestInfo'] = requestinfo;
      disbursalRequest.criteria = criteria
      let disbursalResp = await search_disbursal(disbursalRequest);
      if (!disbursalResp?.Disbursal?.length) {
        return renderError(res, `Failed to query for disbursal search. Check Criteria.`);
      } else {
        disbursals = disbursalResp?.Disbursal;
      }
    } catch (error) {
    ...
    try {
      let csv = await generatePaymentCSVFromDisbursals(disbursals, requestinfo)
      // res.status(201);
      // res.json({
      ...
      filename = 'Payment_Report ' + Date.now() + ".csv"
